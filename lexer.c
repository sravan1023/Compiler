...
/* Global lexer state for simple API */
...
/* Initialize lexer with source code (simple API) */
...
/* Initialize lexer with extended options */
...
static void skip_block_comment(lexer_t *lexer) {
    ...
    lexer_advance(lexer);  /* Skip / initial star */
...
static token_t read_number(lexer_t *lexer) {
    ...
    /* Skip type suffixes (u, l, ul, ll, etc.) */
    ...
    if (is_float) {
        ...
        token.literal.int_val = strtoll(token.value + 2, NULL, 2);  /* Skip 0b */
    } else if (is_octal) {
